...
import { CompanyDialogForm, DeleteDialog, EducationForm, EducationItem, ExperienceForm, ExperienceItem, UserForm } from '../../components';
...
function ProfilePageCompany() {
    ...
    console.log('HFERHFBERJHBGFHRJGJ')
    const handleUpdate = (companyId, updatedCompanyObject) => {
        console.log("updatedCompanyObject",updatedCompanyObject)
        let headers = {
            'Content-type': 'application/json; charset=UTF-8',
            'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
